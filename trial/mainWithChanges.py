...
def connect(botNo=1):
    ...
    serverPort = 12344+botNo
    #create
    server_socket = socket.socket(socket.AF_INET,socket.SOCK_STREAM)

    #bind
    server_socket.bind((IPAddr, serverPort ))
...
def PlotForwardPath(image,current, startpoint, turnpoint, endpoint, direction, bot):
    ...
    if((x1-x3)>50)and not turnedRight:
        #do softright
        client_socket.send(bytes('n', 'utf-8'))
    elif((x1-x3)<-50)and not turnedRight:
        #do softleft
    ...
    while i != robNo:
        while (1):
            ...
            # frame = cv2.imdecode(img_arr, -1)
                ...
                bot = i+1
                if topLeft != (0, 0):
                    PlotForwardPath(vid,topLeft,InitialPoints[i], TurningPoints[i], FinalPoints[i],Directions[i], bot)
                    nextBot = PlotReversePath(vid, topLeft, TurningPoints[i],FinalPoints[i],Directions[i], bot)

                if nextBot == True:
                    #CONNECT TO BOT 1 AT PORT 12345 THEN after its completion close connection and connect to bot 2 at port 12346 and so on
                    client_socket.close()
                    connect(bot)
                    i += 1
                    break
